from protocol.protocolUtils import map_byte_to_address, map_to_byte_address_wchannel


class EleroProtocol():
    flash_table_encode = [0x08, 0x02, 0x0d, 0x01, 0x0f, 0x0e, 0x07, 0x05, 0x09, 0x0c, 0x00, 0x0a, 0x03, 0x04, 0x0b,
                          0x06]
    flash_table_decode = [0x0a, 0x03, 0x01, 0x0c, 0x0d, 0x07, 0x0f, 0x06, 0x00, 0x08, 0x0b, 0x0e, 0x09, 0x02, 0x05,
                          0x04]
    ...
    def add_to_index_table(self, address):
        if address["id"] not in self.indexes_table:
            self.indexes_table[address["id"]] = 1
    ...
    def construct_msg(self, remote_addr, blind_addr, command):
        counter = 1
        b_index = ''.join('{:02X}'.format(a) for a in remote_addr)
        try:
            counter = self.indexes_table[b_index]
            self.indexes_table[b_index] += 1
        except KeyError:
            self.indexes_table[b_index] = counter + 1

        print(f"Status of indexes_table[{b_index}] = {self.indexes_table[b_index]}")
